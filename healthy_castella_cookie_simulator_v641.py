...
if st.button("🚀 執行模擬"):
    ...
    if "高蛋白" in target_goals:
        st.markdown("- ✅ 建議以 **豌豆蛋白粉取代 10% 麵粉或麥芽漿**")
        st.markdown("- ✅ 或可再加入乳清蛋白、大豆分離蛋白強化")

    if "減糖" in target_goals:
        st.markdown("- ✅ 將蔗糖比例從 40% 降至 25% 以下")
        st.markdown("- ✅ 可改為：赤藻糖醇（50%）+ 羅漢果糖（30%）+ 轉化糖漿（20%）")

    if "高纖" in target_goals:
        st.markdown("- ✅ 添加 5% 菊苣纖維或燕麥纖維取代同量糖或麵粉")
        st.markdown("- ✅ 可選用難消化性麥芽糊精補充可溶性纖維")

    if "低GI" in target_goals:
        st.markdown("- ✅ 降低蔗糖比例，改用赤藻糖醇+羅漢果糖")
        st.markdown("- ✅ 使用全穀粉、難消化性麥芽糊精替代部分澱粉原料")
