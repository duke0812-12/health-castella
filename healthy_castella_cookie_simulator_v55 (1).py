...
st.title("長崎蛋糕餅乾健康模擬器 v5.5 🍪（修正版：蛋白質含量82% + 模擬紀錄）")
...
if st.button("🚀 一鍵執行模擬"):
    ...
    st.header("📊 模擬結果預測")
    # 使用正確蛋白質含量：82%
    protein_g = round(cookie_weight * (pea_protein_pct / 100) * 0.82, 2)
    ...
    st.subheader("🍬 糖系組成分析")
    sugar_1 = round(sugar_total_pct * sucrose_ratio / 100, 1)
    sugar_2 = round(sugar_total_pct * erythritol_ratio / 100, 1)
    sugar_3 = round(sugar_total_pct * monk_ratio / 100, 1)
    sugar_4 = round(sugar_total_pct * syrup_ratio / 100, 1)
    st.markdown(f'''
    - 蔗糖：{sugar_1}%  
    - 赤藻糖醇：{sugar_2}%  
    - 羅漢果糖：{sugar_3}%  
    ...
    if "高蛋白" in target_goals and protein_g < 5:
        optimized["pea_protein"] = min(30, round((5 / cookie_weight) / 0.82 * 100, 1))
        notes.append(f"目前每片僅含 {protein_g}g 蛋白，建議豌豆蛋白提高至 {optimized['pea_protein']}%。")
