...
st.title("長崎蛋糕餅乾健康模擬器 v5.5 🍪（修正：豌豆蛋白含量 82%）")
...
cookie_weight = 10
pea_protein_purity = 0.82  # 使用固定值 82%
...
if st.button("🚀 一鍵執行模擬"):
    ...
    st.header("📊 模擬結果預測")
    protein_g = round(cookie_weight * (pea_protein_pct / 100) * pea_protein_purity, 2)
    ...
    if "高蛋白" in target_goals and protein_g < 5:
        required_pct = round((5 / cookie_weight) / pea_protein_purity * 100, 1)
        optimized["pea_protein"] = min(30, required_pct)
